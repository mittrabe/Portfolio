...
class ChessScreen(tk.Frame):
    def __init__(self, parent, controller, boardState, playerColor):
        ...
        for row in range(8):
            for col in range(8):
                ...
                if self.boardState[index] == 'empty':
                    ...
                    piece = ChessPiece(boardState[index],'Pawn',0,0,self.playerColor)
        ...
        def movePiece(start, end):
            start.getPiece().incrementMoves() #increments the number of times the piece has been moved by one
