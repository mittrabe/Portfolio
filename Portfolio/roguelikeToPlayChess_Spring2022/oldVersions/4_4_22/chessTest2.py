...
from shopScreenTest2 import ShopScreen
from chessScreenTest2 import ChessScreen
...
###########################################
    def __init__(self, *args, **kwargs):
        ...
        for F in (TitleScreen, LoginScreen, MenuScreen, ShopScreen, ChessScreen):
            if F == ChessScreen:
                boardState = []
                frame = F(self.window,self, boardState)
            else:
    ...
    def showFrame(self, cont, *args):
        ...
        if cont == 'Chess Screen':
            self.frames[frameIndex] = ChessScreen(self.window, self, args[0])
        elif cont == 'Shop Screen':
            self.frames[frameIndex] = ShopScreen(self.window, self)
