...
class ShopScreen(tk.Frame):
    ...
    def updateUI(self):
        ...
        self.turnsText.configure(text=str(self.numTurns))
    ...
    def updateShopPool(self):
        dummyPiece = ChessPiece('empty', 'Empty', 0, 0,'Empty', self.playerColor, 0)
    ...
    def __init__(self, parent, controller, boardState, numCoins, maxNumCoins, numLives, numWins, numTurns, piecesValue, maxPiecesValue, numRows, playerColor, pieceState): #NEED TO IMPLEMENT # OF GAMES PLAYED
...
#=====================
        ...
        #Saves the locations of each piece in the shop
        def endTurn(self):
            pieceState = []
            for square in self.boardState:
        ...
        def placeCpuPieces():
            for square in self.boardState:
                if square.getPiece().getColor() == "white":
                    square.removePiece()
            randIndex = random.randint(0,15) #randomly place CPU King in first 2 rows
            self.boardState[randIndex].placePiece(ChessPiece(self.cpuColor + 'King', 'King', 0, 0, 'Empty', self.cpuColor, 0))
        ...
        if len(boardState) == 0:
            ...
            for row in range(8):
                for col in range(8):
                    isActive = FALSE
                    piece = ChessPiece('empty', 'Empty', 0, 0,'Empty', self.playerColor, 0)
            ...
            activeSquares[randIndex].placePiece(ChessPiece(self.playerColor + 'King', 'King', 0, 0,'Empty', self.playerColor, 0))
        ...
        def rerollPieces(self, setup): #Rerolls cost 1 coin
            if self.numCoins > 0:
                ...
                for child in shopItemFrame.winfo_children():
                    ...
                    child.placePiece(ChessPiece('black' + self.shopPool[randIndex], self.shopPool[randIndex], 0, 0,'Empty', self.playerColor,0))
