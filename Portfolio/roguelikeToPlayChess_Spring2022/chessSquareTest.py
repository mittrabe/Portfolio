...
class ChessSquare(tk.Button):
    ...
    def __init__(self, parent, piece, row, col, width, height, isActive, board, **kwargs):
        ...
        self.grid(row=self.row+1, column=self.column)
    ...
    def setIsFrozen(self, newFrozen):
        self.isFrozen = newFrozen
    
    def freeze(self):
        self.isFrozen = True
        self.frozenBg = '#A8E4F0'
        self['bg'] = self.frozenBg
    def unfreeze(self):
        self.isFrozen = False
    ...
    def setIsDestroyed(self, status):
        self.isDestroyed = status

    def getFilePath(self, filename):
            return os.path.realpath(__file__)+f'\\..\\{filename}'

    def changePieceOwner(self):
        self.piece.changeOwner()
        self['image'] = self.resizeImage(self.piece.getImage())
    ...
    def removePiece(self):
        ...
        emptySquare = ChessPiece('empty','Empty','Empty',0,0,'Empty','empty',0)
        self.placePiece(emptySquare)
        self.updateMoveset(self, 'move')

    def killPiece(self, board, graveyardData, start):
        if self.getPiece().getType() == "TrojanHorse":
            trojan = ChessPiece(self.getPiece().getColor() + 'Trojan',self.getPiece().getOwner(),'Trojan', 0, 0,'Empty', self.getPiece().getColor(), 0)
            Movesets().getOnDeathAbility(self.getPiece(), self.column,self.row, board, graveyardData,start, trojan)
        else:
            Movesets().getOnDeathAbility(self.getPiece(), self.column,self.row, board, graveyardData,start)
